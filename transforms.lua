...
--end
--
--function M.PersonAugment(imgSize, personProb, personPath)
--    local extensionList = {'png', 'PNG'}
--    local findOptions = ' -iname "*.' .. extensionList[1] .. '"'
--    for i=2,#extensionList do
--        findOptions = findOptions .. ' -o -iname "*.' .. extensionList[i] .. '"'
--    end
--
--    local f = io.popen('find -L ' .. personPath .. findOptions)
--    local personImages = {}
--
--    while true do
--        local line = f:read('*line')
--        if not line then break end
--        table.insert(personImages, line)
--    end
--
--    return function(input)
--        local personSize = torch.random(torch.round(imgSize / 2), torch.round(imgSize * 4 / 5))
--
--        local personTransform = M.Compose{
--            M.ScaleBigger(personSize),
--            M.HorizontalFlip(0.5),
--        }
--
--        if torch.uniform() < personProb then
--            print(personSize)
--
--            local personInd = torch.random(#personImages)
--            local personFilename = personImages[personInd]
--            print(personFilename)
--
--            local person = gm.Image():load(personFilename):toTensor('float', 'RGBA', 'DHW')
--            person = personTransform(person)
--            local personW, personH = person:size(3), person:size(2)
--
--            local person_alpha = person[4]
--            local person_alpha_inv = torch.FloatTensor(1, personH, personW):fill(1):add(-1, person[4])
--            person = person[{{1,3}, {}, {}}]
--
--            local y = input:size(2) - personH
--            local x = torch.random(5, input:size(3) - personW - 5)
--
--            local area = input[{{}, {y, y + personH - 1}, {x, x + personW - 1}}]
--            local alpha_inv = image.scale(person_alpha_inv, personW, personH)
--            area:cmul(alpha_inv:expandAs(area))
--            local alpha = image.scale(person_alpha, personW, personH)
--            local man = image.scale(person, personW, personH)
--            alpha = alpha:view(1, personH, personW)
--            --man = t.colorJitter(man)
--            --print({man})
--            --print({person_alpha})
--            area:add(torch.cmul(man, alpha:expandAs(man)))
--        end
--
--        return input
--    end
